...
export class CommentService {
  ...
  getByComplian(id: number): Observable<EntityArrayResponseType> {
    return this.http
      .get<IComment[]>(`${this.resourceUrl}/by-complain/${id}`, { observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    const copy: IComment = Object.assign({}, comment, {
      createdDate: comment.createdDate && comment.createdDate.isValid() ? comment.createdDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.createdDate = comment.createdDate ? moment(comment.createdDate) : undefined;
      });
    }
    return res;
  }
